...
def test_concatenate_strings_with_join_method(benchmark: Callable, string_list: list[str]):
    """Тестирует производительность метода соединения строк с помощью метода join."""
    assert benchmark(StringConcatenator().concatenate_strings_with_join_method,string_list)== "".join(string_list)


@pytest.mark.parametrize("string_list", samples)
def test_concatenate_strings_with_plus_operation(benchmark: Callable, string_list: list[str]):
    """Тестирует производительность метода соединения строк с помощью оператора +."""
    assert benchmark(StringConcatenator().concatenate_strings_with_plus_operation,string_list)== "".join(string_list)
